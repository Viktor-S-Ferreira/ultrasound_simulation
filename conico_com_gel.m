...
som_aluminio = 5100; % Neste caso, estou usando parametros que mais se aproximam de um gel ou silicone
densidade_aluminio = 7870; % a ideia e diminuir a incompatibilidade do acoplador com a pele ou figado
...
cone_height = (Nx / 2) - 2;       % altura do cone (corresponde à metade do meio)
r1 = Ny / 2;                % raio da base maior do cone
r2 = Ny / 4;                % raio da base menor do cone (metade do maior)

for x = 1:(Nx/2)
    % Garantir que x = 1 e x = (Nx/2)-1 sejam ar
    if x == 1 || x == (Nx/2)
        medium.sound_speed(x, :, :) = som_ar;
        medium.density(x, :, :) = densidade_ar;
    else
