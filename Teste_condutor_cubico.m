...
%medium.alpha_coeff(Nx/2 +1 :Nx, :, :) = 0.75;      % [dB/(MHz^y cm)]
...
input_signal = (source_strength ./ (som_figado * densidade_figado)) .* input_signal;
...
% Coordenadas da seção a ser visualizada
z_slice = 22; % Seção no plano Z = 50

% Plotar densidade
figure;
imagesc(squeeze(medium.density(:, :, z_slice))); % Extraindo a seção em Z
colorbar;
xlabel('X');
ylabel('Y');
